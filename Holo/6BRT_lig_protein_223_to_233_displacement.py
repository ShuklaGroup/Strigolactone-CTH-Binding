...
for file in glob.glob('./lig_stripped/*.strip.dcd'):
    ...
    d = md.compute_displacements(t,[[3408,3554]])     	# distance between the two atoms on helix terminal caps
    n_frames = t.n_frames
    print(np.shape(d))
    ...
    np.save('./lig_analysis/Orientation/'+file.split('/')[-1]+'_223_233_orientation.npy',dis)
