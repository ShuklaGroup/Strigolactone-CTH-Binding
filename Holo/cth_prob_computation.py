...
def compute_eq_contact_probs(contact_distances, dtrajs, msm, cutoff=0.45):
    ...
    cth_contacts = contact_distances[:,270:]
    contacts = np.zeros((np.shape(cth_contacts)[0],1))
    for i in range(np.shape(contacts)[0]):
        if np.any(cth_contacts[i,:] < cutoff):
            contacts[i] = 1
