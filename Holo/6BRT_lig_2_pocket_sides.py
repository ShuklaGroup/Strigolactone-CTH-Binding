...
for file in glob.glob('lig_stripped/*.strip.dcd'):
    ...
    d = md.compute_contacts(t,[[143,158]], scheme='ca')[0]     	# distance between the two residues on helix T3 and T1 respectively
    n_frames = t.n_frames
    ...
    np.save('./lig_analysis/Pocket/'+file.split('/')[-1]+'lig_distance_between_pocket_ends.npy',dis)
