...
if __name__=="__main__": #'__main__' is the name of the python module. Name is set to the name of the script or module
    ...
    ax1_files = sorted(glob.glob('lig_analysis/THelices/*ABC*T2*top*'))
    ax2_files = sorted(glob.glob('lig_analysis/THelices/*D-ring*T2*bot*'))
    ...
    if args.weights == None: #if we didn't provide a file with the weights
        free_energy(ax1_data, ax2_data, lims=(0, 6, 0, 6), max_energy=args.max_energy, label1="A ring to S97 O", label2="D ring to S97 O", savename="rings_to_s97_o.png")
