...
matplotlib.rc('font',family='Helvetica-Normal',size=18) #controls font size, not too hard
...
if __name__=="__main__": #'__main__' is the name of the python module. Name is set to the name of the script or module
    ...
    ax1_files = sorted(glob.glob('lig_analysis/Contacts_for_TICA/*246_to_217*'))
    ax2_files = sorted(glob.glob('lig_analysis/Other_Contacts/*96_to_246*'))
    ...
    if args.weights == None: #if we didn't provide a file with the weights
        free_energy(ax1_data, ax2_data, lims=(args.ax_lims[0], args.ax_lims[1], args.ax_lims[2], args.ax_lims[3]), max_energy=args.max_energy, label1='Resid 218 to 247', label2='Resid 97 to 147', savename=args.savename)
        ...
        free_energy(ax1_data, ax2_data, lims=(args.ax_lims[0], args.ax_lims[1], args.ax_lims[2], args.ax_lims[3]), max_energy=args.max_energy, label1='D218-H247 Distance (nm)', label2='S97-H247 Distance (nm)', savename=args.savename, weights=weights)
