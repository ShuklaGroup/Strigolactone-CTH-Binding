...
if __name__=="__main__": #'__main__' is the name of the python module. Name is set to the name of the script or module
    ...
    datastuff = np.load('TICA_Data_Lig.npy', allow_pickle=True)
    print(np.shape(datastuff))
    print(np.shape(datastuff[0]))
    ...
    TICA = np.vstack([col1, col2, col3, col4, col5, col6]) # col7, col8])
    print(len(TICA))

    all_coords = np.load('Coords_for_Features_Lig.npy', allow_pickle=True)
    print(np.shape(all_coords))
    print(np.shape(all_coords[0]))
    ...
    print(np.shape(funtimes))
    print(np.shape(funtimes[0]))

    feature_label = ['n_terminus_coords', 'c_terminus_coords', 'helicity', 'helix_bend', 'N_266', 'Res229_278',
        'Res130_286', 'D96', 'D195', 'A96', 'D222', 'A222', 'D284', 'B282', '215', '216', '217', '218', '219', '220', '221']

    for i in range(6): #or 4
        for j in range(21): #or 16
            ax1_data = funtimes[j]
            thingy = TICA[i]
            pog = len(thingy)
            ...
            if j == 2:
                ...
                max = 2
            free_energy(ax1_data, ax2_data, lims=(0, max, -8, 8), max_energy=args.max_energy, label1=feature_label[j], label2='TICA_Col{0}'.format(i), savename='TICA_feat{0}_{1}.png'.format(i,j))
