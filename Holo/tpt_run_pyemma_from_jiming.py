...
def id_associated_dissociated_states(samples):  #ask jiming how this one works
    ...
    C_223 = natsorted(glob.glob('lig_analysis/Helices/*c_terminus*'))
    ...
    for i in range(len(samples)):
        ...
        for j in range(len(state_samples)):
            ...
            c_a223_dist.append(np.load(C_223[state_samples[j,0]])[state_samples[j,1]])
        ...
        if all((np.mean(n_a223_dist) > 1.2, np.mean(n_a223_dist) < 1.8, np.mean(c_a223_dist) > 0.8, np.mean(c_a223_dist) < 2.1)):
        #if all((np.mean(t1t3_dist) < 1.3, np.mean(helical) < 0.6, np.mean(t1_ext) < 0.75, np.mean(dloop_dist) > 1.0)):
            associated_states.append(i)
        elif ((np.mean(n_a223_dist) < 1.0 or np.mean(n_a223_dist) > 2.0) and (np.mean(c_a223_dist) < 0.6 or np.mean(c_a223_dist) > 2.3)):
...
if __name__=="__main__":
    ...
    pickle.dump(tpt1, open("tpt_bound_lig.pkl",'wb'))
    print("MFPT:")
    print(mfpt1)
    print(int1)
    print(path1)
