...
def id_anchored_states(samples):
    ...
    A_ring_T1_dist = natsorted(glob.glob("./lig_analysis/Ligand2/*a_ring*137*")) #did this
    D_ring_T4_dist = natsorted(glob.glob("./lig_analysis/Ligand/*d_ring*195*")) #did this
    ...
    for i in range(len(samples)):
        ...
        d_t4 = []
        for j in range(len(state_samples)):
            abc_t1.append(np.load(A_ring_T1_dist[state_samples[j,0]])[state_samples[j,1]])
            d_t4.append(np.load(D_ring_T4_dist[state_samples[j,0]])[state_samples[j,1]])

#FIX THIS PART!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if all((np.min(abc_t1) < 1, np.min(d_t4) < 1.3)):
