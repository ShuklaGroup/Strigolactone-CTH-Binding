...
for file in glob.glob('./lig_stripped/*.strip.dcd'):
    ...
    d = md.compute_contacts(t,[[270,286]], scheme='ca')[0]     	# distance between the two residues on helix T3 and T1 respectively
    n_frames = t.n_frames
    ...
    np.save('./lig_analysis/Helices/'+file.split('/')[-1]+'_distance_between_helix_terminals.npy',dis)
