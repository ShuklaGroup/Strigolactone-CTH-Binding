...
for file in glob.glob('lig_stripped/*.strip.dcd'):
    ...
    d = md.compute_distances(t,[[2138,4408]])     	# distance between the two residues on helix T3 and T1 respectively
    n_frames = t.n_frames
    ...
    np.save('./lig_analysis/Ligand2/'+file.split('/')[-1]+'_GR24_a_ring_to_resid_137.npy',dis)
