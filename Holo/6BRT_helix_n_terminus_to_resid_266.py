...
for file in glob.glob('./lig_stripped/*.strip.dcd'):
    ...
    d = md.compute_contacts(t,[[267,270]], scheme='ca')[0]     	# distance between the two residues on helix T3 and T1 respectively
    n_frames = t.n_frames
    ...
    np.save('./lig_analysis/Other_Contacts/'+file.split('/')[-1]+'_n_terminus_helix_to_resid_266.npy',dis)
