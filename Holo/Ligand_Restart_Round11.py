...
for i in range(len(datafun)):
   ...
   if (y_mins <= datafun[i] <= y_maxs) and (x_mins <= datafun2[i] <= x_maxs):
...
for entry in Final_Points:
    ...
    if Round == 1:
        ...
        if traj_frame.unitcell_volumes == None:
            ...
            traj_frame.save_amberrst7("/home/sobecks2/6BRT/Production_Runs/Ligand/Ligand_Round12/6BRT_apo_helix_GR24_round_12_run_"+str(r))
            print(traj_frame)
            r += 1
    else:
