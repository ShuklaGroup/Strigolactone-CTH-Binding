...
for file in glob.glob('lig_stripped/*.strip.dcd'):
    ...
    d = md.compute_distances(t,[[1491,4423]])     	# distance between the two residues on helix T3 and T1 respectively
    n_frames = t.n_frames
    ...
    np.save('./lig_analysis/Ligand/'+file.split('/')[-1]+'_GR24_d_ring_to_resid_96.npy',dis)
