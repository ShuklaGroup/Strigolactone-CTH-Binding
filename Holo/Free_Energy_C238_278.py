...
if __name__=="__main__": #'__main__' is the name of the python module. Name is set to the name of the script or module
    ...
    ax1_files = sorted(glob.glob('lig_analysis/Other_Contacts/*c_terminus*238.npy'))
    ...
    if args.weights == None: #if we didn't provide a file with the weights
        free_energy(ax1_data, ax2_data, lims=(args.ax_lims[0], args.ax_lims[1], args.ax_lims[2], args.ax_lims[3]), max_energy=args.max_energy, label1='C-Term 238 Distance (nm)', label2='Resid 278 to 222 Distance', savename=args.savename)
