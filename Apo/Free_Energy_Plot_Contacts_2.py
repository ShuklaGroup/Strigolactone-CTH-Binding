...
if __name__=="__main__": #'__main__' is the name of the python module. Name is set to the name of the script or module
    ...
    c_res = [137, 216, 219, 246]
    #n_res = [137, 216, 219, 246]
    res2 = [191, 166, 214, 172]
    i = 0
    for resid1 in c_res:
         ax1_files = sorted(glob.glob('analysis/Other_Contacts/*resid_{0}*'.format(resid1)))
         ax2_files = sorted(glob.glob('analysis/Helices/*c_terminus*'))
    ...
    #print(np.shape(ax2_data))
         ...
         if args.weights == None: #if we didn't provide a file with the weights
              free_energy(ax1_data, ax2_data, lims=(0, 6, 0, 6), max_energy=args.max_energy, label1='Resid{0}_to_{1}'.format(resid1, resid2), label2='C-Term', savename='6BRT_apo_contacts_{0}-{1}_vs_C_Term.png'.format(resid1, resid2))
