...
if __name__=="__main__": #'__main__' is the name of the python module. Name is set to the name of the script or module
    ...
    ax2_files = natsorted(glob.glob('analysis/Helices/*c_terminus*'))
    list_o_tica = [215, 216, 217, 218, 219, 220, 221]
    ...
    for j in list_o_tica:
          ax1_data = []
          ax1_files = natsorted(glob.glob('analysis/Contacts_for_TICA/*resid_246_to_{0}.npy'.format(j)))
          print(len(ax1_files))
          ...
          free_energy(ax1_data, ax2_data, lims=(0, 1.5, 0, 7), max_energy=args.max_energy, label1='D_Loop_{0}'.format(j), label2='C_Terminus (nm)', savename='D_Loop_{0}_C_Term_r15.png'.format(j))
