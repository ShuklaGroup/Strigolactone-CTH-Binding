...
time_series = np.load('time_series_10.npy', allow_pickle=True)
tica_output = np.load('TICA_Data_v3_10.npy', allow_pickle=True)
tica_feature_TIC_correlation = np.load('TICA_Feature_Correlation_v3_10.npy', allow_pickle=True)
coords = np.load('Metric_Coords_Array_v3_10.npy', allow_pickle=True)
...
cluster = pyemma.coordinates.cluster_kmeans(coords, k=350, max_iter=50)
#its = pyemma.msm.its(cluster.dtrajs, lags=[1, 2, 3, 5, 7, 10], nits=3, errors='bayes')
its = pyemma.msm.its(cluster.dtrajs, errors='bayes', lags=800, nits=10)
#we want to see what how fast our processes are
pyemma.plots.plot_implied_timescales(its, outfile="its_plot_apo_10.png", units='ns', dt=.07)

msm = pyemma.msm.estimate_markov_model(cluster.dtrajs, lag=214)
print('fraction of states used = {:.2f}'.format(msm.active_state_fraction))
print('fraction of counts used = {:.2f}'.format(msm.active_count_fraction))

pcca = msm.pcca(10) #number of macrostates, 10 or fewer

pkl.dump(msm, open('msm-model-10.pkl', 'wb'))
pkl.dump(msm.trajectory_weights(), open('msm-traj-weights-10.pkl', 'wb'))
pkl.dump(pcca, open('msm-pcca-10.pkl', 'wb'))
