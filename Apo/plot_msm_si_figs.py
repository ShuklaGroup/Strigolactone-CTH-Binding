...
def plot_microstate_cktest(dtrajs, msm_object, lag_time, mlags=range(4), save_prefix="microstate_cktest"):
    ...
    step_rng = np.array(mlags)*214
    plt.semilogy(np.array(mlags)*214, Tij_n_traces, color='b', linestyle='--', label="Predict")
    #plt.plot(np.array(mlags)*286, Tij_n_traces, color='b', linestyle='--', label="Predict")
    step_rng[0] = 1
    plt.semilogy(np.array(mlags)*214, Tij_est_traces, color='k', label="Estimate")
...
if __name__=="__main__":
    ...
    dtrajs_use = [i for i in dtrajs]
    #dtrajs = pickle.load(open("dtrajs.pkl", 'rb'))
    plot_its(dtrajs_use, 1e6)
    ...
    matplotlib.rc('font',family='Helvetica-Normal',size=20)
    plot_microstate_cktest(dtrajs_use, msm, 214)
