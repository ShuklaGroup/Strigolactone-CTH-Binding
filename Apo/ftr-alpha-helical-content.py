...
for file in glob.glob('./stripped/*initial*strip.xtc'):
       	t=md.load(file,top=targettopfile)
       	val=alpha_helical_content(t,range(270,286))
        print(val)
       	np.save('./analysis/Helical/'+file.split('/')[-1]+'_ftr-alpha-helical-content.npy',val)
