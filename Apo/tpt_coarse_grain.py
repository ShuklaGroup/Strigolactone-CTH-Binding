...
if __name__=="__main__":
    ...
    #WE DID THIS IN THE OTHER CODE samples = get_samples(msm, topfile='stripped.protein_helix_nohydrogen.prmtop')
    #WE DID THIS IN THE OTHER CODE pickle.dump(samples, open("state_samples_apo-10.pkl",'wb'))
    ...
    #pickle.dump(tpt1, open("tpt_bound-10-v2.pkl",'wb'))
    #print("MFPT:")
    #print(mfpt1)
    #print(int1)
    #print(path1)
 
    tpt1 = pickle.load(open('tpt_bound-10-v2.pkl', 'rb'))
    paths, fluxes = tpt1.pathways(fraction=1.0, maxiter=100000)
    print("Total binding flux")
    print(np.sum(fluxes))
    
    #this part creates a network
    sets, tpt_cg = tpt1.coarse_grain([associated, dissociated])
    for s in sets:
        print(s)
    ...
    tptpos = np.array([tpt_cg.committor, [0,0,0]]).transpose()
    fig, axes = mplt.plot_flux(tpt_cg, pos=tptpos, arrow_label_format="%10.4f", attribute_to_plot='net_flux')
    fig.savefig('apo_flux.png', dpi=300)
