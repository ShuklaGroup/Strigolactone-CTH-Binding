...
def get_prob_density(data1, data2, bins=300, binrange=[[0,1],[0,4]], weights=None): #
    ...
    #hist, x_edges, y_edges = np.histogram2d(data1, data2, bins=300, range=binrange, weights=None)

    #prob_density_unweight = hist/np.sum(hist)
    #x_coords = 0.5*(x_edges[:-1]+x_edges[1:]) 
    #y_coords = 0.5*(y_edges[:-1]+y_edges[1:])
    
    #test_density = np.allclose(prob_density_unweight, prob_density)
    #print(test_density)
