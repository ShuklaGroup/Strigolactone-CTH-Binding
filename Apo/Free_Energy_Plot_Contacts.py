...
if __name__=="__main__": #'__main__' is the name of the python module. Name is set to the name of the script or module
    ...
    c_res = [215, 229, 237, 238, 239, 240, 242, 262]
    n_res = [129, 130, 131, 132]

    for resid1 in c_res:

        for resid2 in n_res:

            ax1_files = sorted(glob.glob('analysis/Contacts/*resid_{0}*'.format(resid2)))
            ax2_files = sorted(glob.glob('analysis/Contacts/*resid_{0}*'.format(resid1)))
    ...
    #print(np.shape(ax2_data))
            ...
            if args.weights == None: #if we didn't provide a file with the weights
                free_energy(ax1_data, ax2_data, lims=(0, 6, 0, 6), max_energy=args.max_energy, label1='N-Terminus-nm', label2='C-Terminus-nm', savename='Contact_Images/6BRT_apo_contacts_n_{0}_c_{1}.png'.format(resid2, resid1))
