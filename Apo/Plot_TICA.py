...
if __name__=="__main__": #'__main__' is the name of the python module. Name is set to the name of the script or module
    ...
    #HERE's WHAT DATASTUFF IS NOW!!!
    #consists of 1400 arrays, one for each run
    #each run array has an array given for every single frame
    #the frame array has 4 values, one for each TIC
                ...
                if j >= i:
                    ...
                    for trajectory in datastuff:
                        for frame in trajectory:
                            ax1_data.append(frame[i])
                            ax2_data.append(frame[j])
                    free_energy(ax1_data, ax2_data, lims=(-8, 8, -8, 8), max_energy=args.max_energy, label1='TICA_Col{0}'.format(i), label2='TICA_Col{0}'.format(j), 
                        savename='TICA_{0}_{1}_10tics_r15.png'.format(i,j))
    else:
        ...
        for j in range(6):
            for i in range (6):
                if j >= i:
                    ...
                    for trajectory in datastuff:
                        for frame in trajectory:
                            ax1_data.append(frame[j])
                            ax2_data.append(frame[i])      
                    free_energy(ax1_data, ax2_data, lims=(-2, 4, -3, 3), max_energy=args.max_energy, label1='Apo TIC {0}'.format((j+1)), label2='Apo TIC {0}'.format((i+1)), savename='TICA_{0}_{1}_r15_weighted.png'.format(i,j), weights=weights)
