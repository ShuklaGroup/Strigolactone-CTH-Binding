...
if __name__=="__main__":
    ...
    if le_file[0][2] == 1:
        ...
        roundno = le_file[0][2]
        traj_file = "stripped/6BRT_apo_helix_inactive_round{0}_{1}.strip.xtc".format(roundno, first)
        old_frame = md.load_frame(traj_file, le_file[0][0], top=targettopfile)
        print(le_file[0])
        print(old_frame)
    ...
    for i in range(1, len(le_file)):    
        if le_file[i][2] == 1:
            ...
            q = le_file[i][1]
            roundno = le_file[i][2]
            traj_file = "stripped/6BRT_apo_helix_inactive_round{0}_{1}.strip.xtc".format(roundno, q)
            newest_frame = md.load_frame(traj_file, le_file[i][0], top=targettopfile)
            if newest_frame.unitcell_volumes == None:
                continue
            else:
